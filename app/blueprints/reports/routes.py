...
Data bersumber dari view v_kunjungan_harian di Supabase.
Export: PDF (fpdf2) dan Excel (openpyxl).
"""
...
def _fmt_ts(ts: str | None) -> str:
    """ISO timestamp → 'YYYY-MM-DD HH:MM:SS', atau '-' jika None."""
    if not ts:
        return "-"
    return ts[:19].replace("T", " ")


def _generate_excel(rows: list, tanggal: str, nama_event: str) -> tuple[bytes, str]:
    import openpyxl
...
def _generate_pdf(rows: list, tanggal: str, nama_event: str) -> tuple[bytes, str]:
    ...
    pdf.cell(0, 9, "Laporan Kunjungan — Peken Banyumasan", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 7, f"{nama_event}  |  Tanggal: {tanggal}", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)

    # ── Header tabel ───────────────────────────────────────────────────────
    col_w   = [10, 20, 60, 48, 48, 30, 24]
    headers = ["No", "Tipe", "Nama / Pengunjung", "Waktu Masuk",
               "Waktu Keluar", "Durasi (mnt)", "Status"]
