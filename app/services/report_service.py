...
───────────────────────────
Business logic untuk laporan kunjungan dan export (Excel/PDF).
"""
...
def get_reports(tanggal: str, event_id: str) -> dict:
...
def export_report(fmt: str, tanggal: str, event_id: str) -> tuple[bytes, str, str]:
    """
    Kembalikan (data_bytes, filename, mimetype).
    Validasi parameter dilakukan di router; fungsi ini hanya query + generate.
    """
    supabase = get_supabase()
    try:
        query = (
        ...
        logger.exception("Error generating report export fmt=%s", fmt)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"status": "error", "message": "Terjadi kesalahan pada server"},
        )
...
def _generate_pdf(
    ...
    for i, row in enumerate(rows, start=1):
        ...
        pdf.cell(col_w[1], 7, row.get("tipe_pengunjung", ""),  border=1, align="C", fill=True)
        pdf.cell(col_w[2], 7, nama,                            border=1, fill=True)
        pdf.cell(col_w[3], 7, _fmt_ts(row.get("waktu_masuk")), border=1, fill=True)
        pdf.cell(col_w[4], 7, _fmt_ts(row.get("waktu_keluar")),border=1, fill=True)
        pdf.cell(col_w[5], 7, durasi,                          border=1, align="C", fill=True)
        pdf.cell(col_w[6], 7, row.get("status", ""),           border=1, align="C", fill=True)
        pdf.ln()
